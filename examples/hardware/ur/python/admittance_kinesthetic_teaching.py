...
def main():
    ...
    adm_controller.set_stiffness_matrix_position(np.zeros((3, 3)))
    adm_controller.set_damping_matrix_position(np.identity(3) * 65)

    adm_controller.set_mass_matrix_orientation(np.identity(3) * 0.25)
    adm_controller.set_stiffness_matrix_orientation(np.zeros((3, 3)))
    adm_controller.set_damping_matrix_orientation(np.identity(3) * 5)

    # Uncomment this for the highly damped mode (stable on table)
    # adm_controller.set_damping_matrix_position(np.identity(3) * 65 * 5)
    # adm_controller.set_damping_matrix_orientation(np.identity(3) * 5 * 5)
    ...
    try:
        while True:
            ...
            adm_controller.step(f_base_tip, mu_tip, T_base_tip_pos_init, T_base_tip_quat_init)
